...
/** Work-order attachments: list + download + register (via StorageService
 *  signed-URL flow). No delete — the backend has no delete endpoint. */
export function WorkOrderAttachments({ workOrderId }: { workOrderId: string }) {
  const { hasPermission } = useAuth();
  const { addAttachment } = useWorkOrderMutations(workOrderId);
  const inputRef = useRef<HTMLInputElement>(null);
  const canAdd = hasPermission('workorder:update');

  const q = useQuery({ queryKey: ['work-order', workOrderId, 'attachments'], queryFn: () => living.workOrder.listAttachments(workOrderId) });
  const attachments = q.data ?? [];

  async function onFiles(files: FileList | null) {
    if (!files || files.length === 0) return;
    try {
      for (const file of Array.from(files)) await addAttachment.mutateAsync(file);
      await q.refetch();
    ...
    <div className="flex flex-col gap-3">
            ...
            <li key={a.id} className="flex items-center gap-3 rounded-md px-2 py-2 hover:bg-sunken">
              ...
              {a.downloadUrl && (
                <a href={a.downloadUrl} target="_blank" rel="noreferrer"
                  className="rounded-md p-1.5 text-muted transition-colors hover:bg-tint hover:text-brand" aria-label={`Download ${a.fileName}`}>
      ...
      {canAdd && (
        <div>
          ...
          <Button variant="secondary" size="sm" loading={addAttachment.isPending} onClick={() => inputRef.current?.click()}>
