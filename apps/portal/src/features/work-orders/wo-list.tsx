...
import { WO_KANBAN, WO_STATUSES, WO_TONES, woWorkflow } from './config';
import { WorkOrderForm } from './wo-form';

const PRIORITIES = ['LOW', 'MEDIUM', 'HIGH', 'CRITICAL'] as const;
const ORIGINS = ['MANUAL', 'TICKET', 'SERVICE_REQUEST'] as const;
  ...
  { key: 'number', header: 'Work order', sortKey: 'number', cell: (w) => <span className="font-mono text-sm font-medium text-strong">{w.workOrderNumber}</span> },
  {
    ...
    cell: (w) => (
      <div className="min-w-0">
        ...
        <p className="text-xs text-subtle">{w.unit?.unitNumber ? `Unit ${w.unit.unitNumber}` : 'Common area'} · {w.originType.toLowerCase().replace(/_/g, ' ')}</p>
      </div>
    ),
  },
...
export function WorkOrdersListPage() {
  ...
  return (
    <>
      <ListScaffold
        title="Work orders"
        description="Execute, verify and close operational work."
        query={list}
        columns={columns}
        rowKey={(w) => w.id}
        ...
        filters={[
          ...
          { key: 'originType', placeholder: 'All origins', options: opt(ORIGINS) },
        ]}
